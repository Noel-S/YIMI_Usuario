...
class VerificationCodePhone extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  //Método para actualizar el número del teléfono del usuario.
  updatePhone = async () => {
    try {
      ...
      if (res.status == 200) {
        Alert.alert('Aviso', 'Se ha actualizo tu número teléfono con exito!', [{ text: 'OK', onPress: () => this.props.navigation.navigate('PersonalInformation') }])
      } else {
        Alert.alert('Error', 'Servicio no disponible, intente más tarde.');
      }
    } catch (error) {
            ...
            <Grid>
              ...
              <Row style={{ marginTop: 40 }}>
                <Col style={{ display: 'flex', alignContent: 'center', alignItems: 'center', marginLeft: 85, marginRight: 85 }}>
                  <Button onPress={this.onValidateInput} block style={{ backgroundColor: '#ff8834' }}><Text style={{ fontFamily: 'aller-bd' }}>Siguiente</Text></Button>
                </Col>
              </Row>
            </Grid>
          </Content>
